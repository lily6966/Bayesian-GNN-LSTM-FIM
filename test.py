...
def eval(pred, Y, args):
    ...
    # Remove entries where Y or pred is NA (pred can be NaN from unstable forward passes)
    ...
    with torch.no_grad():
        for batch_idx, (in_nodes, out_nodes, blocks) in enumerate(nodeloader):
            ...
            for i in range(seq_years - 1):  # Exclude current year
                ...
                # Replace NaN labels with year-average (works for any number of outputs)
                year_avg_std = ((year_avg_Y[year_i].to(device) - args.means) / args.stds).squeeze()  # shape (out_dim,)
                for j in range(batch_labels_std.shape[2]):
                    missing_indices = torch.isnan(batch_labels_std[:, i, j])
                    batch_labels_std[missing_indices, i, j] = year_avg_std[j]
...
def test(args):
    ...
    N = adj.shape[0]  # works for both dense ndarray and sparse scipy matrix
    ...
    for year in range(min_year+l-1, max_year+1):
        ...
        X_seqs, Y_seqs, counties = torch.tensor(np.array(X_seqs)), torch.tensor(np.array(Y_seqs)), torch.tensor(counties)
